...
This cap applies to planning ownership, not just implementation.
`50_IMPLEMENTATION/GATE_C_AGENT_CONTEXT_AND_HUMAN_INTERACTION_PLAN.md`'s
refined identity/session/context-envelope model is adopted below (§5.1–5.3)
because it's a genuine improvement on this brief's own original entity list
and fixes a naming mismatch this brief already had between §5 (which used
to say `AgentSession`) and §8 (which already said `RuntimeSession`). What
is **not** adopted from that document: its broader agent roster (Enterprise
Architect, Memory Manager, a dedicated Legal and Regulatory Agent Group as
planning owners) and its legal-source-governance subsystem (its §07) — both
are out of scope until a role beyond the five above is actually approved
for the MVP.
...
Keep these identities separate (refined from
`50_IMPLEMENTATION/GATE_C_AGENT_CONTEXT_AND_HUMAN_INTERACTION_PLAN.md`
§02.6 — this replaces the single `AgentSession` entity the original version
of this section used, and aligns §5 with §8's `RuntimeSession`, which was
already inconsistent with the old §5 wording):

```text
AgentDefinition = enterprise role (e.g. "Execution Agent")
AgentInstance   = one configured, executable instance of that role
Provider        = external or internal model provider
Model           = provider model/version
RuntimeSession  = one temporary execution
```

A provider or model can be swapped without changing `AgentDefinition`
identity. `RuntimeSession` is disposable; `AgentDefinition`/`AgentInstance`
configuration is not.

### 5.2 Context survives session termination

An agent session is disposable; context is not. When a `RuntimeSession`
ends or fails:

```text
RuntimeSession closes
  -> its context snapshot remains immutable
  -> result and evidence stay linked to the task, not the session
  -> an unfinished task remains recoverable
  -> a replacement agent gets a freshly assembled context package
  -> it does not inherit undocumented hidden state from the dead session
```

This rules out relying on provider-side chat history or in-process memory
as the system of record — the durable Bizzi objects (task, context
snapshot, decision, audit record) are.

### 5.3 Provider-neutral context and result envelopes

No provider-specific conversation format becomes the canonical enterprise
memory format. Every agent call goes through a canonical envelope,
converted per-provider by an adapter:

```text
Canonical Context Envelope -> Provider Adapter -> Provider-specific request -> Model execution -> Canonical Result Envelope
```

This is what makes the provider/model swap in §5.1 safe: the same task can
be re-sent to a different provider while the context and comparison stay
traceable. No LLM provider is selected by this brief; provider/model choice
is deliberately deferred (see `docs/planning/TECH_STACK.md`, "Explicitly
not included").
