...
of this build. See ADR-0007 for why the platform-wide "Art of Business"
services are drawn as external/future rather than in-scope containers.

Stack: these diagrams describe a Python + FastAPI + PostgreSQL backend,
per ADR-0007 (supersedes the original NestJS/TypeScript/Prisma scope in
ADR-0002, retained as a historical record only).
...
| C3 — Component | [`C3_COMPONENT.md`](C3_COMPONENT.md) | Python packages/routers inside the API container and how they call each other |
| C4 — Dynamic | [`C4_DYNAMIC_CANONICAL_FLOW.md`](C4_DYNAMIC_CANONICAL_FLOW.md) | The canonical request flow (Router/Endpoint→Service→Repository) for a state-changing call |

## Keeping this current

Update the relevant view in the same PR that:
- adds/removes a container (new deployable service, new datastore) → C2
- adds/removes a Python package/router or changes which services it calls → C3
