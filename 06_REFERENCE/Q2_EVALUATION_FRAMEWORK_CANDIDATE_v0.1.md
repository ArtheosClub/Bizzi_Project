...
> **Standing warning:** This document is not an ADR, ADW decision block, architecture decision, planning authority, or implementation authorization. It must not be cited as authority for a Q2 representation or for any D1–D6 decision below. Its placement under `06_REFERENCE/` is deliberately used to keep the artifact outside the Tier 0–6 authority chain established by `DECISION_0002`. The repository's RKM-01 draft describes the existing Reference layer as unranked and non-authoritative; because RKM-01 itself remains Draft, that classification is supporting repository-organization rationale, not new governance authority created by this file.
...
- WP19 does not inherit a dependency from WP18.
...
`DECISION_0002` establishes the binding Tier 0–6 authority chain. It does not define a research/candidate tier. `docs/planning/` is Tier 4 and therefore carries planning/sequencing standing; `docs/adr/` is Tier 3; `00_ARCHITECTURE/` is Tier 2. Filing this research candidate in any of those locations would make its non-authoritative standing easier to misread.

The repository already contains `06_REFERENCE/`. RKM-01, currently Draft, describes a Reference Layer as unranked and deliberately non-authoritative: material there describes the repository or provides reference context without becoming competing authority. Because RKM-01 is Draft, this file does not claim that model as binding governance. Instead, the placement is the least authority-confusing location available in the existing repository layout, reinforced by the explicit standing warning at the top of this artifact.
