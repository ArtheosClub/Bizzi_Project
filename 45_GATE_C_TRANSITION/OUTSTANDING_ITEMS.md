...
| Supersedes | OIR-01 v1.3 (OI-008 finding widened; no item added, removed, reclassified, or closed) |
...
| 1.4 | OI-008 finding widened — the `GC-*` naming collision confirmed to extend beyond GC-002 to GC-003 and GC-004 (Architecture Decision Proposals vs. Gate C certification milestone documents of the same IDs), found while recording ADR-0010. Proposed rename direction (`GCP-001`–`GCP-010` plus a correspondence table) recorded, not executed. Item remains `Open`/`Reserved`, not closed | OI-008 (finding widened) |
...
| OI-008 | GC-003 §5 (Minor #4); GC-002 §0 (Naming note), EV-C-02 (Comments) | This Gate C Certification Package's own document ID, `GC-002`, collides in form with the pre-existing, unrelated, unapproved `GC-002` Architecture Decision Proposal ("Composite Foreign Keys") in `GATE_C_ARCHITECTURE_DECISION_PROPOSALS.md`. **Confirmed 2026-08-03 to be wider than originally recorded**: `GC-003` and `GC-004` show the identical collision — `40_GATE_C/GC-003_CERTIFICATION_REPORT.md` and `GC-004_APPROVAL_RECORD.md` are unrelated Gate C certification milestone documents colliding with the Architecture Decision Proposals' GC-003 (Membership Invitation Row Occupancy) and GC-004 (Membership Role Model) of the same ID. The collision is presumed to run across the full `GC-001`–`GC-005` range shared by both document sets, not confirmed beyond GC-002/003/004. | Naming | Minor | Open | Reserved | GC-002's own header states resolving the collision is outside that document's scope; this register does not reopen that scope determination. | (none — not closed) | 2026-07-26 | | Repository Maintainer (GC-001 §8 signature role) | Pending | Not applicable | Project Owner | GC-002's own header states resolving the collision is outside that document's scope. This register does not resolve it either. **2026-08-03**: proposed direction recorded, not executed — rename the Architecture Decision Proposals (`GATE_C_ARCHITECTURE_DECISION_PROPOSALS.md`) from `GC-001`–`GC-010` to a separate namespace (e.g. `GCP-001`–`GCP-010`), with a correspondence table preserving historical references. Not performed now: touches ten IDs referenced from the certification documents, the backlog, C4, and this register — comparable in size to a Work Package, and per the Abstraction Justification Rule (`CLAUDE.md`) not a precondition for any currently in-flight WP. See `docs/adr/0010-workspace-membership-mvp-scope.md` Context, which had to navigate this collision directly for GC-003/GC-004. |
...
does not reopen that scope determination. Its finding was widened
2026-08-03 to cover the full `GC-*` namespace (GC-002 through at least
GC-004 confirmed), with a proposed rename direction recorded but not
executed (§3).
